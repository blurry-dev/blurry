...
def parse_python_code(_, match: re.Match, state: BlockState):
...
def render_python_code(_, language: str, path: str):
    package_path, reference_name = path.rsplit(".", 1)
    package = __import__(package_path)
    reference_source = inspect.getsource(getattr(package, reference_name))
